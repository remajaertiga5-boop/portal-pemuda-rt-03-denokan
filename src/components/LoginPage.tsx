...
export default function LoginPage({ anggotaList, onLoginSuccess, onBrowseAsGuest }: LoginPageProps) {
  ...
  const checkLockout = () => { const lockout = checkLockoutStatus(); if (lockout.isLocked) { setLockoutRemaining(lockout.remainingSeconds); return true; } return false; };
  const findMemberById = (cleanId: string): AnggotaItem|undefined => { if (!cleanId) return undefined; return anggotaList.find((m)=>m.ID_Anggota===cleanId && m.Status_Tampil!=="ARSIP"); };
  const validateIdFormat = (id: string) => { return /^(RL03-\d{3}|\d{10})$/.test(id); };

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault(); setErrorMsg(""); setSuccessMsg("");
    const cleanId = idInput.trim(); const cleanPin = pinInput.trim();
    if (!cleanId) { setErrorMsg(lang.error_empty_id); return; }
    if (!cleanPin) { setErrorMsg(lang.error_empty_pin); return; }
    if (checkLockout()) return;
    const foundUser = findMemberById(cleanId);
    if (!foundUser) {
      if (!validateIdFormat(cleanId)) { setErrorMsg(lang.error_id_format); return; }
      addAccessLog(cleanId, "Seseorang", "TAMU", "LOGIN_GAGAL", "ID tidak terdaftar atau diarsip" as any);
      setErrorMsg(lang.error_id_not_found); return;
    }
    if (isNewUser || setupFirstTimePin) {
      if (!/^\d{4,8}$/.test(cleanPin)) { setErrorMsg(lang.error_pin_length); return; }
      if (cleanPin !== confirmPinInput.trim()) { setErrorMsg(lang.error_confirm_mismatch); return; }
      setIsLoading(true);
      try {
