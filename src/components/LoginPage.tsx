...
export default function LoginPage({ anggotaList, onLoginSuccess, onBrowseAsGuest }: LoginPageProps) {
  ...
  const [loading, setLoading] = useState(false); const [lr, setLr] = useState(0); const [setupPin, setSetupPin] = useState(false);
  const idRef = useRef<HTMLInputElement>(null); const pinRef = useRef<HTMLInputElement>(null);
  const L = LOGIN_I18N[currentLanguage] || LOGIN_I18N.id;
  ...
  const go = async (e: React.FormEvent) => {
    ...
    const u = find(cid);
    if (!u) {
      if (!/^(RL03-\d{3}|\d{10})$/.test(cid)) { setErr(L.error_id_format); return; }
      addAccessLog(cid, "Seseorang", "TAMU" as any, "LOGIN_GAGAL" as any, "ID tidak terdaftar");
      setErr(L.error_id_not_found); return;
    }
    if (isNewUser || setupPin) {
      ...
      try {
        hashPin(cpin);
        const s = saveAuthSession({ status:"active",id_anggota:u.ID_Anggota,nama_lengkap:u.Nama_Lengkap,nama_panggilan:u.Nama_Panggilan||u.Nama_Lengkap,role:getRoleFromJabatan(u.Jabatan||""),jabatan:u.Jabatan,login_time:new Date().toISOString(),remember_me:true } as any);
        addAccessLog(cid, u.Nama_Lengkap, getRoleFromJabatan(u.Jabatan) as any, "PIN_SETUP" as any, "Setup PIN baru");
        setOk(L.success_login); setTimeout(()=>onLoginSuccess(s), 1000);
      } catch { const a = recordFailedPinAttempt(cpin); setErr(L.wrong_pin); if (a>=3) setLr(60); }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50 to-teal-50 dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950 flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="mt-4 text-center text-[10px] text-slate-400 dark:text-slate-500">{L.organization_name} — {L.organization_loc}<br/><a href="#" className="text-emerald-600 font-bold"><Phone size={12} className="inline"/> {L.help_call}</a></div>
      </div>
    </div>
  );
}
