...
// 🔒 SPREADSHEET ID & PROXIES — TERKUNCI DI SERVER (TIDAK BISA OVERRIDE CLIENT)
// Menolak spreadsheetId / sheetsId dari request client untuk mencegah pengaksesan sheet acak
var CURRENT_SPREADSHEET_ID = null;

// Helper Compute Hash SHA-256 untuk PIN
...
function handleRequest(e) {
  ...
  try {
    ...
    // 🔒 SPREADSHEET ID — Selalu gunakan ID dari server (ScriptProperties / CONFIG)
    // Abaikan parameter spreadsheetId dari client untuk keamanan
    var scriptProps = PropertiesService.getScriptProperties();
    CURRENT_SPREADSHEET_ID = scriptProps.getProperty('SPREADSHEET_ID') || CONFIG.SPREADSHEET_ID;
...
// ✅ FIXED: isPinValid — validasi bertingkat (ScriptProperties → Sheet Pengaturan → CONFIG Default)
function getStoredPinOrHash_(level) {
  var props = PropertiesService.getScriptProperties();
  var propKey = 'PIN_' + level;
  var hashKey = 'PIN_' + level + '_HASH';
  
  // 1. Cek ScriptProperties (Plain text atau SHA-256 Hash)
  var propVal = props.getProperty(hashKey) || props.getProperty(propKey);
  if (propVal && propVal.trim()) return propVal.trim();

  // 2. Cek Sheet 'Pengaturan' di Google Sheets
  try {
    ...
    if (sheet) {
      ...
      for (var i = 0; i < values.length; i++) {
        ...
        if (key === propKey || key === ('PIN_' + level + '_DEFAULT') || (level === 'SUPER_ADMIN' && key === 'PIN_ADMIN')) {
    ...
    // Abaikan jika sheet Pengaturan belum siap
  }

  // 3. Fallback ke CONFIG
  switch (level) {
    case 'PENGURUS'   : return CONFIG.PIN_PENGURUS_DEFAULT;
    case 'KETUA'      : return CONFIG.PIN_KETUA_DEFAULT;
    case 'SUPER_ADMIN': return CONFIG.PIN_SUPER_ADMIN;
    default           : return null;
  }
}

// Cek jam dinamis YYYYMMDDHH (offset -1, 0, +1)
function checkDynamicPin_(pinInput) {
  if (!pinInput || String(pinInput).length !== 10) return false;
  var now = new Date();
  for (var offset = -1; offset <= 1; offset++) {
    var t = new Date(now.getTime() + offset * 3600000);
    ...
    var dynPin = yyyy + mm + dd + hh;
    if (String(pinInput) === dynPin) return true;
  }
  return false;
}
...
function isPinValid(pin, level) {
  ...
  var inputStr = String(pin).trim();
  var stored = getStoredPinOrHash_(level);

  if (stored) {
    var storedStr = String(stored).trim();
    // Cocokkan langsung (plaintext PIN)
    if (inputStr === storedStr) return true;
    // Cocokkan SHA-256 Hash
    if (hashPin_(inputStr) === storedStr) return true;
  }

  // Verifikasi PIN Dinamis untuk SUPER_ADMIN jika format 10-digit
  if (level === 'SUPER_ADMIN' && inputStr.length === 10) {
    if (checkDynamicPin_(inputStr)) return true;
  }
